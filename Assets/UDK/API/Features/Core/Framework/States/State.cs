namespace UDK.API.Features.Core.Framework.StateLib
{
    ...
    using System.Linq;
    using UDK.API.Features.Core.Framework.Attributes;

    /// <summary>
    /// The base class which handles in-context states.
    /// </summary>
    public abstract class State : UActor, IState
    {
        private static readonly List<State> statesValue = new();

        protected readonly HashSet<StateController> controllers = new();
        protected readonly HashSet<StateController> activeControllers = new();
        protected readonly HashSet<StateController> inactiveControllers = new();
        ...
        private static void RuntimeInitializeStates() => InitializeStates();
        ...
        {
            foreach (Type type in Assembly.GetExecutingAssembly().GetTypes().Where(t => t.IsClass && !t.IsAbstract))
            {
                ...
                if (useAttribute)
                {
                    ...
                    if (stateAtt is null)
                    {
                        Log.Info($"A {nameof(State)} cannot be initialized due to missing {nameof(StateAttribute)} attribute");
                        continue;
                    }
                }
                ...
                {
                    Log.Info($"Couldn't initialize the state cannot be initialized due to missing {nameof(StateAttribute)} attribute");
                    continue;
                }
